...
export class StarrocksSqlBuilder extends WarehouseBaseSqlBuilder {
    readonly type = WarehouseTypes.STARROCKS;
...
export class StarrocksWarehouseClient extends WarehouseBaseClient<CreateStarrocksCredentials> {
    ...
    constructor(credentials: CreateStarrocksCredentials) {
        super(credentials, new StarrocksSqlBuilder(credentials.startOfWeek));
    ...
    async streamQuery(
        ...
        try {
            ...
            const [rows, fields] = await session.query<RowDataPacket[]>(
                alteredQuery,
                options.values,
            );
        ...
        await Promise.all(
            requests.map(async (request) => {
                try {
                    ...
                    rows.forEach((row) => {
                        const catalog = row.table_catalog ?? request.database;

                        if (!warehouseCatalog[catalog]) {
                            warehouseCatalog[catalog] = {};
                        }
                        if (!warehouseCatalog[catalog][row.table_schema]) {
                            warehouseCatalog[catalog][row.table_schema] = {};
                        }
                        ...
                        warehouseCatalog[catalog][row.table_schema][
                            ...
                            convertStarrocksDataTypeToDimensionType(
                                row.data_type,
                            );
                    });
